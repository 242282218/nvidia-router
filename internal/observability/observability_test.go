...
func TestRepositoryListDailyCostsJoinsModelPricing(t *testing.T) {
	db := openObservabilityDB(t)
	repository := NewRepository(db)
	ctx := context.Background()

	// A priced model with token traffic must yield a priced cost row.
	if _, err := db.Exec(`
		...
		VALUES ('meta/llama-3.1-8b-instruct', 'upstream/priced', 'Priced', 'chat', 1,
		        0.14, 0.28, '2026-08-01T00:00:00Z', '2026-08-01T00:00:00Z')
	`); err != nil {
		t.Fatalf("insert priced model: %v", err)
	}
	...
	if _, err := db.Exec(`
		...
		VALUES ('2026-08-10', 'model', 'meta/llama-3.1-8b-instruct', 2, 2, 0, 1000000, 1000000)
	`); err != nil {
		t.Fatalf("insert model daily stats: %v", err)
	}
	_ = first
	_ = second

	since := time.Date(2026, 8, 10, 0, 0, 0, 0, time.UTC)
	costs, err := repository.ListDailyCosts(ctx, since, since.Add(24*time.Hour))
	if err != nil {
		t.Fatalf("ListDailyCosts: %v", err)
	}
	if len(costs) != 1 {
		t.Fatalf("costs length = %d, want 1: %#v", len(costs), costs)
	}
	row := costs[0]
	if row.ModelID != "meta/llama-3.1-8b-instruct" || !row.Priced {
		t.Fatalf("cost row = %#v", row)
	}
	if row.PromptTokens != 1_000_000 || row.CompletionTokens != 1_000_000 {
		t.Fatalf("cost tokens = %d/%d, want 1000000/1000000", row.PromptTokens, row.CompletionTokens)
	}
	// 1M prompt @ $0.14 + 1M completion @ $0.28
	if row.InputCostUSD-0.14 > 1e-6 || row.OutputCostUSD-0.28 > 1e-6 || row.TotalCostUSD-0.42 > 1e-6 {
		t.Fatalf("cost USD = in:%f out:%f total:%f, want 0.14/0.28/0.42", row.InputCostUSD, row.OutputCostUSD, row.TotalCostUSD)
	}
}

func TestRepositoryListDailyCostsSkipsUnPricedModelsButReportsPricedFalse(t *testing.T) {
	db := openObservabilityDB(t)
	repository := NewRepository(db)
	ctx := context.Background()

	// No models row at all: the LEFT JOIN keeps the daily_stats row and reports
	// Priced=false so the UI can prompt for missing prices.
	if _, err := db.Exec(`
		INSERT INTO daily_stats (day, dimension_type, dimension_id, request_count, success_count, failure_count,
		                        prompt_tokens, completion_tokens)
		VALUES ('2026-08-10', 'model', 'some/unpriced-model', 1, 1, 0, 500000, 250000)
	`); err != nil {
		t.Fatalf("insert unpriced model daily stats: %v", err)
	}
	...
	if len(costs) != 1 {
		t.Fatalf("costs length = %d, want 1", len(costs))
	}
	row := costs[0]
	if row.Priced {
		t.Fatalf("unpriced model reported Priced=true")
	}
