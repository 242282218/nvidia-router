...
func TestClassifierResponseTable(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "explicit invalid key", status: 403, body: `{"error":{"code":"invalid_api_key"}}`, scope: ScopeCredential, disableKey: true},
		{name: "explicit auth type", status: 403, body: `{"error":{"type":"authentication_error"}}`, scope: ScopeCredential, disableKey: true},
		{name: "account deactivated", status: 403, body: `{"error":{"code":"account_deactivated"}}`, scope: ScopeCredential, disableKey: true},
