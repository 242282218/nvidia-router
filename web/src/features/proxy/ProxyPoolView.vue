...
function buildPatch(): ProxyPoolPatch { return { enabled: enabled.value, validation_url: (validationUrl.value ?? '').trim(), validation_status: validationStatus.value ?? 404, interval: (interval.value ?? '5s').trim(), proxy_ttl: (proxyTTL.value ?? '120s').trim(), expected_qty: expectedQty.value ?? 2, concurrency: concurrency.value ?? 2, max_latency: (maxLatency.value ?? '').trim() } }
async function clearUpstreamURL(): Promise<void> {
  if (!settings.value?.upstream_configured || saving.value) return
  if (enabled.value) { formError.value = '清除 XApi 地址前请先停用内置代理池。'; return }
...
<template>
  <div class="page-container animate-fade-in"><div class="content-wrapper">
    ...
    <form v-else class="card p-5 animate-slide-up" novalidate @submit.prevent="save">
      <div class="flex flex-wrap items-start justify-between gap-4"><div><h2 class="text-sm font-medium">运行配置</h2><p class="mt-1 text-sm text-[var(--color-text-muted)]">当前配置来源：{{ sourceLabel(settings?.source) }}。XApi 凭据只以密文保存，页面不回显完整地址。</p></div><label class="flex min-h-11 items-center gap-3 text-sm"><input v-model="enabled" data-testid="proxy-enabled" class="h-4 w-4 accent-[var(--color-accent)]" type="checkbox"><span>启用内置代理池</span></label></div>
      <div class="mt-6 grid gap-5 md:grid-cols-2"><div class="block text-sm font-medium md:col-span-2"><span>XApi 上游地址</span><div data-testid="proxy-upstream-summary" class="input-field mt-1.5 font-mono">{{ settings?.upstream_configured ? `已配置（${settings.upstream_endpoint || 'endpoint 已脱敏'}）` : '未配置' }}</div><input v-model="upstreamURL" data-testid="proxy-upstream-url" class="input-field mt-2 font-mono" type="password" autocomplete="new-password" placeholder="粘贴完整 XApi URL（保存后不回显）"><span id="proxy-upstream-help" class="mt-1 block text-xs text-[var(--color-text-muted)]">管理端可修改，完整地址只发送到后端加密保存；清除凭据前必须先停用代理池。</span></div><label class="block text-sm font-medium"><span>验证地址</span><input v-model="validationUrl" data-testid="proxy-validation-url" class="input-field mt-1.5 font-mono" type="url" placeholder="留空使用 NVIDIA API 根地址"></label><label class="block text-sm font-medium"><span>预期状态码</span><input v-model.number="validationStatus" data-testid="proxy-validation-status" class="input-field mt-1.5" type="number" min="100" max="599"></label><label class="block text-sm font-medium"><span>采集周期</span><input v-model="interval" data-testid="proxy-interval" class="input-field mt-1.5 font-mono" placeholder="5s"></label><label class="block text-sm font-medium"><span>代理 TTL</span><input v-model="proxyTTL" data-testid="proxy-ttl" class="input-field mt-1.5 font-mono" placeholder="120s"></label><label class="block text-sm font-medium"><span>采集并发</span><input v-model.number="concurrency" data-testid="proxy-concurrency" class="input-field mt-1.5" type="number" min="1" max="20"></label><label class="block text-sm font-medium"><span>最大延迟（可选）</span><input v-model="maxLatency" data-testid="proxy-max-latency" class="input-field mt-1.5 font-mono" placeholder="例如 3s"></label></div>
       <p v-if="formError" class="mt-4 text-sm text-[var(--color-danger)]" role="alert">{{ formError }}</p><p v-if="savedMessage" class="mt-4 inline-flex text-sm badge-success px-3 py-1">{{ savedMessage }}</p><div class="mt-6 flex flex-wrap justify-end gap-3"><button data-testid="proxy-refresh-now" class="btn-secondary" type="button" :disabled="refreshing || !settings?.enabled" @click="refreshPool">{{ refreshing ? '采集中…' : '立即采集' }}</button><button data-testid="proxy-clear-upstream" class="btn-secondary" type="button" :disabled="saving || !settings?.upstream_configured || enabled" @click="clearUpstreamURL">清除 XApi 凭据</button><button data-testid="proxy-save" class="btn-primary" type="submit" :disabled="saving || !settings">{{ saving ? '保存中…' : '保存配置' }}</button></div>
    </form>
