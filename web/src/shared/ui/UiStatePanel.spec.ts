...
describe('UiStatePanel', () => {
  it('accepts kebab-case test-id attrs the way templates pass them', () => {
    const Host = defineComponent({
      components: { UiStatePanel },
      template: '<UiStatePanel error="失败。" errorTestId="host-error" retryTestId="host-retry" />',
    })
  ...
  it('renders a skeleton outline instead of a spinner while loading', () => {
    const wrapper = mount(UiStatePanel, {
      props: { loading: true, skeleton: 'table' },
    })
