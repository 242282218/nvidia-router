...
<template>
  ...
  >
    <button
      v-for="tab in tabs"
      :key="tab.id"
      class="flex h-8 items-center gap-1.5 rounded-[var(--radius-control)] px-3 text-[13px] font-medium transition-[background-color,color,box-shadow] duration-[var(--duration-micro)]"
      :class="active === tab.id
